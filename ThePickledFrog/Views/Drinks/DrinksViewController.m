...
- (void)viewDidLoad
{
    ...
    NSArray *drinksArray = [dataSource getAllDrinks];
    
    if(fromMap == 1){
        // Need to update the currentIndex somehow
        for (int i = 0; i < [drinksArray count]; i++)
        {
            Drink *tempDrink = drinksArray[i];
            if ([mapDrink.drinkName isEqualToString:tempDrink.drinkName])
            {
                currentIndex = i;
                break;
            }
...
//    NSString *bodyFont = [configurationValues objectForKey:@"BodyFont"];
    ...
    // Get Photos
    NSArray *photos = [dataSource getPhotoNames:@"Drink" identifier:drink.drinkName];
    int displayValue = 0;
    ...
    NSString *drinkName = drink.drinkName;
    drinkName = [drinkName stringByReplacingOccurrencesOfString:@"'" withString:@""];
    ...
    NSAttributedString *attributedString = [[NSAttributedString alloc] initWithString:drinkName attributes: attributesHeading];
    ...
    // Opening Hours
    if ([drink.openingHours length] != 0){
        attributedString = [[NSAttributedString alloc] initWithString:drink.openingHours attributes: attributesHeading];
    ...
    NSAttributedString *attributedDescription = [[NSAttributedString alloc] initWithString:drink.description attributes: attributes];
    ...
    if ([drink.description2 length] != 0){
        attributedDescription = [[NSAttributedString alloc] initWithString:drink.description2 attributes: attributes];
    ...
    if ([drink.description3 length] != 0){
        attributedDescription = [[NSAttributedString alloc] initWithString:drink.description3 attributes: attributes];
    ...
    if ([drink.gettingThere length] != 0){
        ...
        headingLabel.font = [UIFont fontWithName:@"OpenSans-CondensedBold" size:fontSize];
        
        [aboutView addSubview:headingLabel];
        
        aboutPosition = aboutPosition + 8 + headingLabel.frame.size.height;
        
        attributedDescription = [[NSAttributedString alloc] initWithString:drink.gettingThere attributes: attributes];
    ...
    if ([drink.closestStop length] != 0){
        ...
        NSString *cs = [drink.closestStop stringByAppendingString:@" "];
    ...
    // Name Label
    attributedString = [[NSAttributedString alloc] initWithString:drinkName attributes: attributesHeading];
    ...
    if ([drink.address length] > 0) {
        ...
        NSAttributedString *attributedAddress = [[NSAttributedString alloc] initWithString:drink.address attributes: attributes];
    ...
    // Currency Converter
    imageValue = @"currencytoolbar.png";    UIImage *currencyImage = [UIImage imageNamed:imageValue];
...
// Set the Drink object sent by the map
...
- (IBAction)loadTALink
{
    NSString *urlToDisplay = drink.taLink;
...
{
    NSString *urlToDisplay = drink.instaURL;
...
{
    NSString *urlToDisplay = drink.facebookURL;
...
- (IBAction)loadDrinkMap
{
    MapViewController *map = [[MapViewController alloc] initWithNibName:@"MapViewController" bundle:nil];
    [map setLatitude:drink.latitude];
    [map setLongitude:drink.longitude];
    [map setDrink:drink];
    [self.navigationController pushViewController:map animated:YES];
}
